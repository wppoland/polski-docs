...
Модуль бухгалтерських інтеграцій з'єднує WooCommerce з популярними польськими системами виставлення рахунків-фактур: wFirma, Fakturownia та iFirma. Рахунки-фактури надсилаються автоматично після оформлення замовлення, з механізмом повторних спроб (retry) та експоненціальною затримкою (exponential backoff) у разі помилок API.
