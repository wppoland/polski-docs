---
title: Час доставки
description: Налаштування часу доставки для кожного продукту та варіанту, значення за замовчуванням та таксономія polski_delivery_time у WooCommerce.
---

Директива Omnibus та польський Закон про права споживачів вимагають, щоб інтернет-магазин інформував про передбачуваний час доставки продукту перед оформленням замовлення. Плагін Polski for WooCommerce забезпечує гнучку систему управління часом доставки - від глобальних налаштувань за замовчуванням до індивідуальних значень для кожного продукту та варіанту.

## Правові вимоги

Продавець повинен повідомити споживача про термін доставки товару не пізніше моменту, коли споживач виявляє волю укласти договір (тобто перед натисканням кнопки замовлення). Ця інформація повинна бути:

- зрозумілою та чіткою
- вказана у робочих або календарних днях
- видимою на сторінці продукту

Відсутність інформації про час доставки може призвести до штрафів з боку UOKiK та становить порушення прав споживача.

## Таксономія polski_delivery_time

Плагін реєструє спеціалізовану таксономію `polski_delivery_time`, яка дозволяє створювати попередньо визначені терміни доставки та призначати їх продуктам.

### Управління термінами доставки

Перейдіть до **Продукти > Час доставки**, щоб керувати доступними термінами.

Приклади термінів:

| Назва | Slug | Опис |
|-------|------|------|
| 1-2 робочі дні | 1-2-dni-robocze | Продукти на складі |
| 3-5 робочих днів | 3-5-dni-roboczych | Продукти, що замовляються у постачальника |
| 7-14 робочих днів | 7-14-dni-roboczych | Продукти на замовлення |
| До 24 годин | do-24-godzin | Цифрові продукти / експрес |
| Доступний одразу | dostepny-od-reki | Негайна реалізація |

Терміни доставки працюють як таксономія WordPress - ви можете їх створювати, редагувати та видаляти в адміністративній панелі.

## Налаштування

### Глобальні налаштування

Перейдіть до **WooCommerce > Налаштування > Polski > Ціни** та налаштуйте розділ "Час доставки".

| Налаштування | Опис |
|--------------|------|
| Увімкнути час доставки | Активує відображення на сторінці продукту |
| Час доставки за замовчуванням | Резервне значення (fallback) для продуктів без призначеного терміну |
| Показати у лістингу | Відображає час доставки на сторінках категорій |
| Показати у кошику | Відображає час доставки у кошику |
| Мітка | Текст перед часом доставки (за замовчуванням: "Час доставки:") |

### Fallback за замовчуванням

Час доставки за замовчуванням (fallback) відображається, коли продукт не має призначеного індивідуального терміну. Це дозволяє швидко впровадити функцію без необхідності редагування кожного продукту окремо.

Ієрархія відображення:

1. Час доставки варіанту (якщо встановлений)
2. Час доставки основного продукту (якщо встановлений)
3. Час доставки за замовчуванням з глобальних налаштувань (fallback)

Якщо жодне з вищезазначених значень не встановлено, інформація про час доставки не буде відображена.

### Призначення продукту

У редакторі продукту, у вкладці "Доставка", ви знайдете поле **Час доставки**. Виберіть з переліку існуючий термін або створіть новий.

### Призначення варіанту

Для змінних продуктів кожен варіант може мати власний час доставки. Розгорніть секцію варіанту та встановіть індивідуальний термін. Варіанти без встановленого терміну успадковують значення від основного продукту.

## Shortcode

Використовуйте shortcode `[polski_delivery_time]`, щоб відобразити час доставки у будь-якому місці.

### Параметри

| Параметр | Тип | За замовчуванням | Опис |
|----------|-----|------------------|------|
| `product_id` | int | поточний | ID продукту |
| `label` | string | `"Czas dostawy: "` | Мітка перед значенням |
| `show_label` | bool | `true` | Чи відображати мітку |
| `wrapper` | string | `span` | HTML-елемент обгортки |
| `fallback` | string | `""` | Текст, коли час доставки відсутній |

### Приклади використання

Базове використання:

```html
[polski_delivery_time]
```

Результат: `Czas dostawy: 1-2 dni robocze`

Без мітки:

```html
[polski_delivery_time show_label="false"]
```

Результат: `1-2 dni robocze`

З власною міткою та fallback:

```html
[polski_delivery_time label="Wysyłka: " fallback="Zapytaj o dostępność"]
```

Для конкретного продукту:

```html
[polski_delivery_time product_id="456"]
```

У шаблоні PHP:

```php
echo do_shortcode('[polski_delivery_time product_id="' . $product->get_id() . '"]');
```

## Програмне управління часом доставки

### Призначення терміну продукту

```php
wp_set_object_terms($product_id, '1-2-dni-robocze', 'polski_delivery_time');
```

### Отримання терміну продукту

```php
$terms = wp_get_object_terms($product_id, 'polski_delivery_time');
if (!empty($terms) && !is_wp_error($terms)) {
    $delivery_time = $terms[0]->name;
}
```

### Створення нового терміну

```php
wp_insert_term(
    '2-3 dni robocze',
    'polski_delivery_time',
    [
        'slug'        => '2-3-dni-robocze',
        'description' => 'Standardowy czas realizacji',
    ]
);
```

## Імпорт CSV

Для імпорту часу доставки через CSV використовуйте колонку:

| Колонка CSV | Опис | Значення |
|-------------|------|----------|
| `polski_delivery_time` | Назва терміну доставки | `1-2 dni robocze` |

Якщо термін із зазначеною назвою не існує, він буде автоматично створений під час імпорту.

Приклад:

```csv
"Laptop Dell XPS 15",5499.00,"3-5 dni roboczych"
"Mysz Logitech MX",299.00,"1-2 dni robocze"
```

## Динамічний час доставки

Для продуктів із тривалим терміном реалізації можна програмно змінювати відображуваний час доставки на основі залишків на складі або дати замовлення.

```php
add_filter('polski/delivery_time/display', function (string $delivery_time, WC_Product $product): string {
    if ($product->get_stock_quantity() > 0) {
        return '1-2 dni robocze';
    }

    return '7-14 dni roboczych';
}, 10, 2);
```

## Стилізація CSS

```css
.polski-delivery-time {
    display: inline-block;
    margin-top: 0.5em;
    font-size: 0.9em;
    color: #2e7d32;
}

.polski-delivery-time__label {
    font-weight: 600;
}

.polski-delivery-time__value {
    color: #333;
}
```

## Найпоширеніші проблеми

### Час доставки не відображається

1. Перевірте, чи модуль увімкнено в налаштуваннях
2. Переконайтеся, що продукт має призначений термін або встановлено fallback за замовчуванням
3. Перевірте, чи тема підтримує hook `woocommerce_single_product_summary`

### Час доставки варіанту не змінюється після вибору

Переконайтеся, що JavaScript плагіна завантажений. Перевірте консоль браузера на наявність помилок JS. Плагін оновлює час доставки варіанту через AJAX при зміні опції.

## Пов'язані ресурси

- [Повідомити про проблему](https://github.com/wppoland/polski/issues)

<div class="disclaimer">Ця сторінка має виключно інформаційний характер і не є юридичною консультацією. Перед впровадженням зверніться до юриста. Polski for WooCommerce - це програмне забезпечення з відкритим кодом (GPLv2), що надається без гарантій.</div>
